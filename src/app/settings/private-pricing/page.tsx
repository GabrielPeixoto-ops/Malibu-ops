  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Private Pricing</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          <p className="text-sm">No rates yet. Add one to get started.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Trucks</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Size</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Men</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">$/hr</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {rates.map((r, i) => (
                <tr key={r.id} className={`hover:bg-gray-50 transition-colors ${!r.is_active ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-gray-600">{r.trucks}</td>
                  <td className="px-4 py-3 text-gray-600 capitalize">{r.truck_size}</td>
                  <td className="px-4 py-3 text-gray-600">{r.men}</td>
                  <td className="px-4 py-3 text-right font-mono text-gray-900">${Number(r.rate_per_hour).toFixed(2)}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(r)} className="p-1.5 text-gray-400 hover:text-blue-600 rounded">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(r)} className="p-1.5 text-gray-400 hover:text-red-600 rounded">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Input
              ...
              min="1"
              value={form.trucks}
              onChange={(e) => setField('trucks', e.target.value)}
            />
            <Select
              label="Truck Size"
              options={truckSizeOptions}
              value={form.truck_size}
              onChange={(e) => setField('truck_size', e.target.value as 'small' | 'large')}
            />
            <Input
              label="Men"
              type="number"
              min="1"
              value={form.men}
              onChange={(e) => setField('men', e.target.value)}
            />
          </div>
          <Input
